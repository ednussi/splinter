...
for d in 'hotpotqa' 'naturalquestions' 'newsqa' 'searchqa' 'textbookqa' 'triviaqa' 'squad'
do
  ...
  do
    ...
    do
      echo "Loop $i-$j"
      EXPNAME="$d-coherent-text"
      TRAINFILE="$BASEPATH/mrqa_data/$d/$d-train-seed-$j-num-examples-$i.jsonl"
      TESTFILE="$BASEPATH/mrqa_data/$d/dev.jsonl"
      OUTPUTDIR="/cs/snapless/gabis/ednussi/results/$EXPNAME/output-$i-$j"
      AUGMENTATION="concat-coherent-text"
      mkdir -p -- $OUTPUTDIR
